...
Automatic power-button wake is enabled by default when `PIN_PWR_BUTTON_OUT` is configured. The firmware waits 60 seconds after it observes the PC shut down before arming standby Bluetooth scanning or wake pulses; if the Pico boots while the PC is already off, standby scanning arms immediately. During that wait the Pico 2 W onboard LED slow-blinks; once standby scanning is active it stays on; in normal USB Bluetooth dongle mode it is off. After a wake match, the firmware drives the power-button line low for 200 ms, releases it with pull-up, then waits 10 seconds before trusting the power LED/sense input again, so the PC has time to start and bring the LED/sense line high. Use `-DENABLE_POWER_BUTTON_WAKE=OFF` for bring-up if the power-button circuit or sense input is not ready. Use `-DSTANDBY_WAKE_ARM_DELAY_MS=<ms>` to tune the post-shutdown arm delay.
